...
class AcessLog():
    def __init__(self):
        ...
        request, endpoint = self.generate_request()
        self.request_line = request
        code, code_type = self.generate_status_code()
        ...
        if code_type == "redirect":
            print(Fore.WHITE + self.log)
            redirect_log = AcessLog()
            print(Fore.GREEN + redirect_log.log)
        elif code_type in ["client_error", "server_error"]:
            error_log = ErrorLog(self, endpoint)
...
while True:
    if random.random() < 0.3: 
        ...
        print(f"--- Starting burst of {burst_length} logs ---")
